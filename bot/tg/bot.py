...
def show_categories(user_id: int, chat_id: int, users_data: dict[int, dict[str | int, ...]]) -> str:
    ...
    users_data[chat_id] = {index: item.cat_id for index,
                           item in enumerate(data, start=1)}
    users_data[chat_id]['next_handler'] = choose_category

    message = [f'{index}) {item.title}' for index,
               item in enumerate(data, start=1)]
...
def create_goal(**kwargs) -> str:
    ...
    try:
        ...
        Goal.objects.create(title=message, user_id=user_id,
